...
Completion criteria: Provide a concrete, verified implementation strategy with exact file modifications required. Check the current state of `PlayerModel.ts`, `PlayerController.ts`, `EnemyManager.ts` and `EnemyModel.ts` and how AnimationMixer should be integrated (update loops, extracting animations from GLTF). Do NOT implement the code yourself.
