...
export class EnemyModel {
  ...
  constructor() {
    ...
    const modelGroup = Game.instance.assetManager.models['enemy'];
    if (modelGroup) {
      ...
      console.warn('[EnemyModel] Model not found in AssetManager');
    }
  }
  ...
  public playState(state: string) {
    ...
    if (!matchedAction) {
      ...
      if (key.includes('walk') || key.includes('run')) matchedAction = this.actions['walk'] || this.actions['run'] || this.actions['walking'];
